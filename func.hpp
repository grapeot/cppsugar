...
namespace func {
    ...
    template<typename TIn>
	static size_t ArgMin(const std::vector<TIn> &in) {
    ...
    template<typename TIn, typename TOut>
	static size_t ArgMin(const std::vector<TIn> &in, const std::function<TOut(TIn)> f) {
            return ArgMin(Map<TIn, TOut>(in, f));
        }

    template<typename TIn>
	static size_t ArgMax(const std::vector<TIn> &in) {
            if (in.empty()) throw std::runtime_error("No element in the sequence.");
            size_t maxi = 0;
            for (size_t i = 1; i < in.size(); i++) {
                if (in[i] > in[maxi]) {
    ...
    template<typename TIn, typename TOut>
	static size_t ArgMax(const std::vector<TIn> &in, const std::function<TOut(TIn)> f) {
            return ArgMax(Map<TIn, TOut>(in, f));
        }

    template<typename TIn>
	static TIn First(const std::vector<TIn> &in, std::function<bool(TIn)> f, const TIn &def = TIn()) {
            for (auto i : in) {
                if (f(i)) {
                    return i;
                }
    ...
    template<typename T>
	static bool All(const std::vector<T> &in, const std::function<bool(T)> f) {
             for (auto i : in) {
                 if (!f(i)) return false;
             }
             return true;
